...
function(env_node_resolve_checksums_version _version _installed_version
         _fallback_version _out)
...
function(env_node_fetch_checksums_file _url _name _out)
  ...
  string(JOIN / _file "${${LOWER_PROJECT_NAME}_checksums_fetch_src_dir}"
         "${_name}")
...
set(__env_node_checksums_headers_archive_regex
    ...
    CACHE STRING "Regex for matching NodeJS components from checksums file."
          FORCE)
...
function(
  ...
  file(
    STRINGS "${_checksums_file}" _checksum_line
  ...
  string(REGEX MATCH "${__env_node_checksums_headers_archive_regex}"
               _checksum_match "${_checksum_line}")
  ...
  set(${_out_checksum}
      "${_checksum}"
      PARENT_SCOPE)
  env_log(NodeJS headers' archive checksum is \"${_checksum}\".)

  string(JOIN "" _rpath "${_version}/"
...
function(env_node_extract_win_lib32_components _checksums_file _out_checksum
         _out_version_rpath)
  ...
  string(REGEX MATCH "${__env_node_checksums_win_lib32_regex}" _checksum_match
               "${_checksum_line}")
...
function(env_node_extract_win_lib64_components _checksums_file _out_checksum
         _out_version_rpath)
  ...
  string(REGEX MATCH "${__env_node_checksums_win_lib64_regex}" _checksum_match
               "${_checksum_line}")
...
function(env_node_add_os_specific_interface _win_delay_load_hook_src
         _node_win_lib)
  ...
  if(ENV_WIN)
    env_use_lower_project_name()
    add_library(${LOWER_PROJECT_NAME}_win_delay_load_hook OBJECT
    ...
    env_target_link_with(os INTERFACE "-IGNORE:4199" "-DELAYLOAD:iojs.exe"
                         "-DELAYLOAD:node.exe" "-DELAYLOAD:node.dll")

  else()
    env_target_definitions(os INTERFACE "_LARGEFILE_SOURCE"
...
function(env_node_decorate_bind _name)
  ...
  env_target_set(${_mod} SUFFIX ".node" MACOSX_RPATH ON)

  string(TOUPPER "${_mod}_BUILD" _build_def)
  env_target_definitions(${_mod} PRIVATE "MODULE__name=${_mod}" "${_build_def}")

  env_macro_name(${_mod} _prefix)
  env_target_definitions(${_mod} PRIVATE "${_prefix}_NODE_BIND=1"
                         "${UPPER_PROJECT_NAME}_NODE_BIND=1" "ENV_NODE_BIND=1")
