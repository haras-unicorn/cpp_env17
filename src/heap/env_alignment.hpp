...
ENV_BENCH(aligned_storage_heap)
{
    ENV_BLOOP
    {
        ...
        ENV_STD::transform(vec.begin(), vec.end(), vec.begin(), [](auto s) { return uint32_t{s} + 3; });
        let res = ENV_STD::all_of(vec.begin(), vec.end(), [](auto s) { ret uint32_t{s} == 4; });
...
ENV_BENCH(aligned_int_heap)
{
    ENV_BLOOP
    {
        ...
        ENV_STD::fill(vec.begin(), vec.end(), 1);
        ENV_STD::transform(vec.begin(), vec.end(), vec.begin(), [](auto i) { ret i + 3; });
        let res = ENV_STD::all_of(vec.begin(), vec.end(), [](auto i) { ret i == 4; });
