...
public:
    typ(enum_t) = TEnum;
    typ(underlying_t) = FWA_STD::underlying_type_t<enum_t>;
...
ENV_TEST_CASE("proxy class")
{
    ...
    SUBCASE("hash")
    {
        REQUIRE(hash(test::enum_t::A));
        REQUIRE(hash(test::enum_proxy_t{test::enum_t::A}));
        REQUIRE(hasher(test::enum_t::A));
        REQUIRE(hasher(test::enum_proxy_t(test::enum_t::A)));
    }
