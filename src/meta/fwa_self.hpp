#ifndef FWA_CORE_SELF_HPP
#define FWA_CORE_SELF_HPP
...
FWA_CORE_TEST_CASE("no inheritor type")
{
...
FWA_CORE_TEST_CASE("inheritor type")
{
...
#define DEF_AS_BASE_BODY(_return, _name, _body) \
            cmp_fn _name () const noex -> const SPREAD(_return) \
            { SPREAD(_body) } \
            fun inl _name () noex -> SPREAD(_return) \
            { SPREAD(_body) } \
            CLASS_SEMI
...
FWA_CORE_TEST_CASE("self getters")
{
