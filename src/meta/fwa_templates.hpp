#ifndef FWA_CORE_TEMPLATES_HPP
#define FWA_CORE_TEMPLATES_HPP
...
FWA_CORE_TEST_CASE("template skips")
{
    REQUIRES(SKIP_COND);
...
FWA_CORE_TEST_CASE("expression conversion")
{
    strct
    {
...
COND_TMP_BINARY
(FWA_STD::is_same_v < TLhs, void_t > && FWA_STD::is_same_v < TRhs, void_t >)
...
COND_TMP_TERNARY
(FWA_STD::is_same_v < T1, void_t > && FWA_STD::is_same_v < T2, void_t > && FWA_STD::is_same_v < T3, void_t >)
...
COND_TMP_VARIADIC
(FWA_STD::conjunction_v < FWA_STD::is_same < TVar, void_t >...>)
cmp_fn void_variadic() noex { ret true; }

COND_TMP_VARIADIC
(FWA_STD::disjunction_v < FWA_STD::negation < FWA_STD::is_same < TVar, void_t >>...>)
cmp_fn void_variadic() noex { ret false; }

FWA_NAMESPACE_TEST_END

FWA_CORE_TEST_CASE ("require template")
{
    SUBCASE("unary")
    {
...
FWA_CORE_TEST_CASE ("expr template")
{
    SUBCASE("unary")
    {
