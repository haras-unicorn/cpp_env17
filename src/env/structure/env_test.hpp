...
#if defined(ENV_SKIP_TESTS) || defined(ENV_SKIP_TESTS) || \
        (defined(ENV_STATIC_TESTS) && !defined(ENV_TESTS))

    #define ENV_TEST_SUITE_BEGIN(_name) \
        DOCTEST_TEST_SUITE_BEGIN(_name* ENV_DTEST::skip(true))

    #define ENV_TEST_SUITE_END() \
        DOCTEST_TEST_SUITE_END()

    #define ENV_TEST_CASE(_name) \
        DOCTEST_TEST_CASE(_name* ENV_DTEST::skip(true))

    #define ENV_TEST_CASE_CLASS(_name) \
        DOCTEST_TEST_CASE_CLASS(_name* ENV_DTEST::skip(true))

    #define ENV_TEST_CASE_TMP(_name, _tmp, ...) \
        DOCTEST_TEST_CASE_TEMPLATE(_name* ENV_DTEST::skip(true), _tmp, __VA_ARGS__)

#else // ENV_SKIP_TESTS

    #define ENV_TEST_SUITE_BEGIN(_name) \
        DOCTEST_TEST_SUITE_BEGIN(_name)

    #define ENV_TEST_SUITE_END() \
        DOCTEST_TEST_SUITE_END()

    #define ENV_TEST_CASE(_name) \
        DOCTEST_TEST_CASE(_name)

    #define ENV_TEST_CASE_CLASS(_name) \
        DOCTEST_TEST_CASE_CLASS(_name)

    #define ENV_TEST_CASE_TMP(_name, _tmp, ...) \
        DOCTEST_TEST_CASE_TEMPLATE(_name, _tmp, __VA_ARGS__)
