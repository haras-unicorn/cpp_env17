...
COND_TMP((exc_name, name TFrom, name TTo), ENV::is_placeable_on_g<const ENV::subject_gt <TFrom>&, TTo>)
...
ENV_TEST_CASE("copy")
{
    ...
    REQUIRES_FALSE(noexcept(copy(&a, scast<const int*>(nullptr), &b, throw_ex)));
...
ENV_TEST_CASE("overlapping copy")
{
    int a[]{1, 2, 3};
    ocopy(&a[0], &a[2], &a[1]);
    REQUIRES_FALSE(noexcept(ocopy(a + 0, scast<int*>(nullptr), a + 1, throw_ex)));
...
COND_TMP((exc_name, name TAlloc, name TFrom, name TTo), ENV::is_placeable_on_g<const ENV::subject_gt <TFrom>&, TTo>)
...
ENV_TEST_CASE("uninitialized copy")
{
    ...
    REQUIRES_FALSE(noexcept(ucopy(alloc, source, scast<value_t*>(nullptr), dest_begin, throw_ex)));
    REQUIRE_THROWS(ucopy(alloc, source, scast<value_t*>(nullptr), dest_begin, throw_ex));
...
ENV_TEST_CASE("move")
{
    ...
    REQUIRES_FALSE(noexcept(move(&a, scast<const int*>(nullptr), &b, throw_ex)));
...
ENV_TEST_CASE("overlapping move")
{
    int a[]{1, 2, 3};
    omove(&a[0], &a[2], &a[1]);
    REQUIRES_FALSE(noexcept(omove(a + 0, scast<int*>(nullptr), &a[1], throw_ex)));
...
COND_TMP((exc_name, name TAlloc, name TFrom, name TTo), ENV::is_writeable_to_g<const ENV::subject_gt <TFrom>&, TTo>)
...
ENV_TEST_CASE("uninitialized move")
{
    ...
    REQUIRES_FALSE(noexcept(umove(alloc, source, scast<value_t*>(nullptr), dest_begin, throw_ex)));
    REQUIRE_THROWS(umove(alloc, source, scast<value_t*>(nullptr), dest_begin, throw_ex));
...
COND_TMP((exc_name, name TLhs, name TRhs), ENV::are_interchangeable_g < TLhs, TRhs >)
...
COND_TMP((exc_name, name TAlloc, name TIter, name... TVar), ENV::is_emplaceable_g < TIter, ENV::v < TVar...>>)
