...
ENV_TEST_CASE("application formula body")
{
    struct test_t
    {
        ...
        inline constexpr static int return_1_formula()
        {
            ret SPREAD(APP_FML(return_1, (())));
        }

        inline constexpr static int return_1_body()
        {
            SPREAD(FML_BODY((return_1())));
        }
    };

    REQUIRES(test_t::return_1() == 1);
    REQUIRES(test_t::return_1_formula() == 1);
    REQUIRES(test_t::return_1_body() == 1);
}
