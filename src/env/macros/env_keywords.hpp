...
#else // ENV_CPP >= 17
    #define sass(...) \
        static_assert(__VA_ARGS__, STRING(Assertion #__VA_ARGS__ failed.))
    #define sass_msg \
        static_assert
#endif // ENV_CPP >= 17
...
#define op operator
    ...
    #define dtyp(...) decltype(auto)
#else // ENV_CPP >= 14
    #define deduc     auto
    #define dtyp(...) __VA_ARGS__
#endif // ENV_CPP >= 14
...
ENV_TEST_CASE("keyword keywords")
{
    ...
    [[OBJECT_ATTRIBUTES]] auto _i =
            1;

    [[OBJECT_ATTRIBUTES]] const auto _scast =
            scast<int>(_i);

    [[OBJECT_ATTRIBUTES]] const auto _ccast =
            ccast<const int&>(_scast);

    [[OBJECT_ATTRIBUTES]] const auto _rcast =
            rcast<const ENV_STD::byte*>(&_ccast);
