...
COND_CHECK_UNARY(
        ...
        is_std_legacy_iterator_gs<T>,

        is_std_swappable_gs<decl(*declval<T>())>);
...
COND_CHECK_UNARY(
        ...
        /* weirdly, there is a difference between
         * the requirement of constructing an lvalue or an rvalue which
        ...
        ENV_STD::is_convertible<decl(declval<T>() == nullptr), bool>,
        ENV_STD::is_convertible<decl(nullptr == declval<T>()), bool>,
        ENV_STD::is_convertible<decl(declval<T>() != nullptr), bool>,
        ENV_STD::is_convertible<decl(nullptr != declval<T>()), bool>);
...
COND_CHECK_UNARY(
        ...
        ENV_STD::is_object<T>,
        is_callable_gs<T>,
        neg_vt<is_callable_member_gs<T>>);
...
COND_CHECK_UNARY(is_std_callable, is_callable_gs<T>);
...
COND_CHECK_UNARY(
        is_std_hash,
        is_std_function_object_gs<T>,
        is_std_copy_constructible_gs<T>,
        is_std_destructible_gs<T>,

        /* I can't check anything else, other than if it's callable, because
         * in order to get the return type,
         * I have to know the key type because it could be overloaded
         * which is impossible  */
        /* there are some more requirements other than
         * the return type being a std::size_t */
        is_callable_gs<T>,
        neg_vt<is_callable_member_gs<T>>);
...
// Predicate, BinaryPredicate and Compare are
// impossible to check and the reason is the same as it is with hash
