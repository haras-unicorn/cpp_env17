...
strct container_traits_gt
{
    ...
    sass(ENV_STD::is_same_v<
            name ENV_STD::iterator_traits<iterator_t>::difference_type,
            difference_t>);
    sass(ENV_STD::is_same_v<
            name ENV_STD::iterator_traits<c_iterator_t>::difference_type,
            difference_t>);

    /* size_t must be large enough to
     * represent all positive values of difference_t */
...
public:
    ...
    sass(implies(is_std_copy_assignable_g<value_t>,
                 is_std_copy_insertable_into_g<value_t, T>));
    sass(implies(is_std_move_assignable_g<value_t>,
                 is_std_move_insertable_into_g<value_t, T>));
...
public:
    ...
    sass(ENV_STD::is_same_v<
            ENV_STD::reverse_iterator<iterator_t>,
            reverse_iterator_t>);
    sass(ENV_STD::is_same_v<
            ENV_STD::reverse_iterator<c_iterator_t>,
...
strct unordered_associative_container_traits_gt :
    public allocator_aware_container_traits_gt<T>
{
private:
    typ(container_traits_t) = container_traits_gt<T>;
...
public:
    ...
    sass(iterator_traits_gt<local_iterator_t>::
                 tmp is_same_g<iterator_traits_gt<iterator_t>>);
    sass(is_std_legacy_iterator_g<c_local_iterator_t>);
    sass(iterator_traits_gt<c_local_iterator_t>::
                 tmp is_same_g<iterator_traits_gt<c_iterator_t>>);
};
...
EXPR_CHECK_UNARY(
        ...
        T(declval<ENV_STD::initializer_list<
                  name container_traits_gt<T>::value_t>>()));
...
COND_CHECK_UNARY(
        ...
        ENV_STD::is_same<
                decl(declvalr<T>().insert(
                        declval<name container_traits_gt<T>::c_iterator_t>(),
                        declval<ENV_STD::initializer_list<
                                name container_traits_gt<T>::value_t>>())),
                name container_traits_gt<T>::iterator_t>);
...
COND_CHECK_UNARY(
        ...
        ENV_STD::is_same<decl(declvall<T>() = declvall<const T>()), T&>);
...
COND_CHECK_UNARY(
        ...
        ENV_STD::is_convertible<decl(declval<T>() == declval<T>()), bool>,
        ENV_STD::is_convertible<decl(declval<T>() != declval<T>()), bool>);
...
EXPR_CHECK_UNARY(
        ...
        T(declval<ENV_STD::initializer_list<
                  name unordered_associative_container_traits_gt<T>::value_t>>(),
          declval<name unordered_associative_container_traits_gt<T>::size_t>(),
          declval<name unordered_associative_container_traits_gt<T>::hasher_t>(),
          declval<name unordered_associative_container_traits_gt<T>::key_equal_t>()));
