...
/* there is a lot of constraints for the methods */
        ...
        (!is_std_move_assignable_g < T > ) &&
        (!is_std_move_constructible_g < T > )
);
...
(
        ...
        COND_EXPR(is_std_shared_mutex_g<T> && is_std_timed_mutex_g<T>),
