...
COND_CHECK_UNARY(
        is_indirect,
        ENV_STD::is_object<ENV_STD::remove_reference_t<decl(*declvall<T>())>>);

COND_CONCEPT(indirect, is_indirect_gs<C>);
...
COND_CONCEPT_UNARY(subject_of, same_vt<C, subject_gt<T>>);
...
COND_CONCEPT_UNARY(pointing_to, points_to_gs<C, T>);
