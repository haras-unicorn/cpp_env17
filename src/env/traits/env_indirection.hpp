...
COND_CHECK_UNARY(is_indirect, ENV_STD::is_object_v < ENV_STD::remove_reference_t < decl(*declvall<T>()) >>);
...
ENV_TEST_CASE("indirect")
{
    ...
    REQUIRES_FALSE(ENV_STD::is_object_v < void() >);
    REQUIRE_EQT(void(), ENV_STD::remove_reference_t < void(&)() >);
    ...
    // MSVC: REQUIRE_EQT(void(*)(), decl(*declval<void (*)()>()));
    REQUIRES_FALSE(ENV_STD::is_object_v < ENV_STD::remove_reference_t < void(&)() >>);
...
tmp<name T> typ(subject_gt) = ENV_STD::remove_reference_t<decl(*declvall<indirect_r<T>>())>;
