...
public:
    ...
    tmp<name = requirement_t> cmp_obj static bool is_supported
            {!_self_t::is_noex && !_self_t::is_member};
};
...
strct callable_gs<
        ...
        (
                ...
                (ENV::detail::callable_gs <
                 ENV::unqualified_gt < TQ >> ::is_callable) &&
                (!ENV::has_call_operator_g < ENV::unqualified_gt < TQ >> )
        )> :
        public callable_gs<
...
strct callable_gs<
        ...
        ENV::success_vt<
                ...
                (
                        ...
                        !ENV::detail::is_non_overloaded_functor_g <
                        TFunctor >
                )>>
{
    EXPR_CLASS_CHECK_VARIADIC(is_callable,
...
tmp<name T>
cmp_obj ENV_STD::size_t argument_count_g =
        detail::callable_gs<T>::argument_count;

tmp<name T, ENV_STD::size_t Index>
...
tmp<name T>
cmp_obj bool is_supported_callable_g
        {detail::callable_gs<T>::tmp is_supported<>};
...
ENV_TEST_CASE("callable traits")
{
    ...
    SUBCASE("check")
    {
        ...
        (is_callable_g<void (*)()>);
        REQUIRES
        (is_callable_g<void(*)() noex >);

        REQUIRES
        (is_callable_g<test_t&>);
        REQUIRES_FALSE
        (is_callable_g<const test_t>);
        REQUIRES
        (is_callable_g<test::templated_callable_t>);
        REQUIRES
        (is_callable_g<ENV_STD::function < void()>>);
        REQUIRES
        ...
        REQUIRES_FALSE
        (is_callable_g<const volatile ENV_STD::function<void() >>);

        REQUIRES
        (is_callable_g<decl(&test_t::op())>);
        REQUIRES
        ...
        REQUIRES
        (is_callable_g<decl(&test::templated_callable_t::lambda)>);
        REQUIRES_FALSE
        (is_callable_g<decl(&test::templated_callable_t::lambda_ptr)>);
    ...
    SUBCASE("ret")
    {
        ...
        (is_callback_g<void()>);
        REQUIRES
        (is_callback_g<void(int)>);
        REQUIRE_EQT
        (return_gt<decl(&test_t::int_unqualified_except_int)>, int);
        REQUIRE_EQT
        (return_gt<decl(&test_t::lvalue_except_char)>, void);
        REQUIRE_EQT
        (return_gt<test_t>, float);
    }
    SUBCASE("arguments")
    {
        REQUIRES
        (is_consumer_g<void(int)>);
        REQUIRES
        (is_consumer_g<int(int)>);
        REQUIRES
        (argument_count_g<decl(&test_t::const_except_double_float)> == 2);
        REQUIRES
        (argument_count_g<decl(&test_t::op())> == 2);
        REQUIRE_EQT
        (argument_tuple_gt<decl(&test_t::int_unqualified_except_int)>,
         ENV_HANA::tuple < int >);
        REQUIRE_EQT
        (argument_tuple_gt<decl(&test_t::const_except_double_float)>,
         ENV_HANA::tuple < double, float >);
    }
    SUBCASE("function")
    {
        REQUIRE_EQT
        (function_gt<decl(&test_t::int_unqualified_except_int)>,
         int(int));
        REQUIRE_EQT
        (function_ptr_gt<decl(&test_t::const_except_double_float)>,
         void(*)(double, float));
        REQUIRE_EQT
        (function_ptr_gt<decl(&test_t::char_unqualified_noex)>,
         char(*)());
    }
    SUBCASE("noex")
    {
        REQUIRES
        (is_noex_callable_g<decl(&test_t::char_unqualified_noex)>);
        REQUIRES
        (is_noex_callable_g<decl(&test_t::volatile_rvalue_noex)>);
        REQUIRES_FALSE
        (is_noex_callable_g<test_t>);
        REQUIRES_FALSE
        (is_noex_callable_g<decl(&test_t::op())>);
    }
    SUBCASE("member")
    {
        REQUIRES
        (is_callable_member_g<decl(&test_t::char_unqualified_noex)>);
        REQUIRES_FALSE
        (is_callable_member_g<decl(&test_t::static_except)>);
    }
    SUBCASE("supported")
    {
        REQUIRES_FALSE
        (is_supported_callable_g<decl(&test_t::char_unqualified_noex)>);
        REQUIRES
        (is_supported_callable_g<decl(&test_t::static_except)>);
    }
    SUBCASE("holder")
    {
        REQUIRE_EQT
        (callable_holder_gt<decl(&test_t::char_unqualified_noex)>, test_t);
    }
    SUBCASE("qualifiers")
    {
        REQUIRES
    ...
    {
        REQUIRE_EQT
        (return_gt<decl(&test::templated_callable_t::op() < void > )>, void);
        ENV_STD::invoke(
