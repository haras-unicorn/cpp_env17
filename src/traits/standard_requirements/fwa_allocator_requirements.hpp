#ifndef FWA_CORE_ALLOCATOR_REQUIREMENTS_HPP
#define FWA_CORE_ALLOCATOR_REQUIREMENTS_HPP
...
(
        ...
        (FWA_STD::is_class_v<T> ? has_arrow_operator_g<T> : FWA_STD::is_pointer_v<T>)
);

// using a custom struct because we want to make sure that TAlloc is not templated with it already
strct rebind_t {};
tmp<name T> typ(rebound_alloc_gt) = name FWA_STD::allocator_traits<T>::tmp rebind_alloc<rebind_t>;
tmp<name T> typ(rebound_traits_gt) = name FWA_STD::allocator_traits<T>::tmp rebind_traits<rebind_t>;


tmp<name TAlloc>
strct allocator_traits_gt : public FWA_STD::allocator_traits<TAlloc>
{
private:
    typ(std_traits_t) = FWA_STD::allocator_traits<TAlloc>;
...
public:
...
#else // FWA_CPP >= 17
    typ(always_equal_t) = FWA_STD::false_type;
#endif // FWA_CPP >= 17
    ...
    sass(FWA_STD::is_same_v<decl(*declval<ptr_t>()), value_t&>);

    sass(is_std_allocator_ptr_g<c_ptr_t>);
    sass(FWA_STD::is_same_v<decl(*declval<c_ptr_t>()), const value_t&>);
    sass(FWA_STD::is_convertible_v<ptr_t, c_ptr_t>);

    sass(is_std_nullable_pointer_g < void_ptr_t > );
    sass(EXPR_COND(scast<ptr_t>(declval<void_ptr_t>())));
    sass(FWA_STD::is_convertible_v<ptr_t, void_ptr_t>);
    sass(FWA_STD::is_same_v<void_ptr_t, name rebound_traits_gt<alloc_t>::void_pointer>);
    ...
    sass(FWA_STD::is_convertible_v<ptr_t, c_void_ptr_t>);
    sass(FWA_STD::is_convertible_v<c_ptr_t, c_void_ptr_t>);
    sass(FWA_STD::is_convertible_v<void_ptr_t, c_void_ptr_t>);
    sass(FWA_STD::is_same_v<c_void_ptr_t, name rebound_traits_gt<alloc_t>::const_void_pointer>);

    sass(!FWA_STD::is_const_v<value_t> && !FWA_STD::is_volatile_v<value_t>);
    sass(FWA_STD::is_object_v<value_t>);

    // can represent the size of the largest object
    sass(FWA_STD::is_unsigned_v<size_t> && !FWA_STD::is_floating_point_v<size_t>); // unsigned integer

    // can represent the difference of any two pointers
    sass(FWA_STD::is_signed_v<difference_t> && !FWA_STD::is_floating_point_v<difference_t>); // signed integer

    // interestingly it is only optional if the allocator takes more or less than one template type parameters
    sass(FWA_STD::is_same_v<name rebound_traits_gt<alloc_t>::tmp rebind_alloc<value_t>, alloc_t>);
};
...
typ(test_allocator_t) = FWA_STD::vector<int>::allocator_type;
typ(test_alloc_traits_t) = FWA_STD::allocator_traits<test_allocator_t>;
typ(test_alloc_ptr_t) = FWA_STD::allocator_traits<FWA_STD::vector<int>::allocator_type>::pointer;

FWA_NAMESPACE_TEST_END

FWA_CORE_TEST_CASE("allocator")
{
...
#endif // FWA_CORE_ALLOCATOR_REQUIREMENTS_HPP
