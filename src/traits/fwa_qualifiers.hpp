#ifndef FWA_CORE_QUALIFIERS_HPP
#define FWA_CORE_QUALIFIERS_HPP
...
FWA_STD::conditional_t<
        RefType == ref_type_t::lvalue, add_lvalue_ref_gt<remove_ref_gt<T>>, FWA_STD::conditional_t<
                RefType == ref_type_t::rvalue, add_rvalue_ref_gt<remove_ref_gt<T>>,
                remove_ref_gt<T>>>;
...
FWA_CORE_TEST_CASE("ref qualifier")
{
    SUBCASE("check")
    {
        SUBCASE("lvalue")
...
FWA_CORE_TEST_CASE("const qualifier")
{
    SUBCASE("check")
    {
...
FWA_CORE_TEST_CASE("volatile qualifier")
{
    SUBCASE("check")
    {
...
FWA_CORE_TEST_CASE("cv qualifiers")
{
    SUBCASE("remove")
    {
    ...
    SUBCASE("copy")
    {
        REQUIRE_EQT(copy_cv_ggt<const double, const volatile int&&>, const int&&);
...
tmp<name TFrom, name TTo>
typ(copy_qualifiers_ggt) = set_qualifiers_gnnnt<TTo, is_const_g<TFrom>, is_volatile_g<TFrom>, ref_type_g<TFrom>>;

FWA_CORE_TEST_CASE("all qualifiers")
{
    SUBCASE("remove")
    {
    ...
    SUBCASE("set")
    {
        REQUIRE_EQT(set_qualifiers_gnnnt<const volatile int&&, true, false, ref_type_t::lvalue>, const int&);
        REQUIRE_EQT(set_qualifiers_gnnnt<const int&, false, true, ref_type_t::rvalue>, volatile int&&);
        REQUIRE_EQT(set_qualifiers_gnnnt<volatile int, true, true, ref_type_t::none>, const volatile int);
    }
    SUBCASE("copy")
    {
        REQUIRE_EQT(copy_qualifiers_ggt<const double, const volatile int&&>, const int);
        REQUIRE_EQT(copy_qualifiers_ggt<volatile double&&, const int&>, volatile int&&);
        REQUIRE_EQT(copy_qualifiers_ggt<const volatile float&, volatile int>, const volatile int&);
    }
}
...
FWA_CORE_TEST_CASE("qualified for")
{
    strct test_t { };
...
//    REQUIRES(is_qualified_for_g<volatile test_t, test_t>);
//    REQUIRES(is_qualified_for_g<const volatile test_t, const test_t>);
//    REQUIRES(is_qualified_for_g<volatile test_t, volatile test_t>);
//    REQUIRES(is_qualified_for_g<const volatile test_t, volatile test_t>);
    REQUIRES(is_qualified_for_g<test_t&, test_t>);
    REQUIRES(is_qualified_for_g<test_t&&, test_t>);
    REQUIRES_FALSE(is_qualified_for_g<volatile test_t&&, test_t>); // loses volatile on construction
...
FWA_CORE_TEST_CASE("forward qualified for")
{
    strct test_t { test_t(const test_t&) = delete; test_t(test_t&&) = delete; };
...
FWA_CORE_TEST_CASE("call qualified for")
{
    strct test_t { test_t(const test_t&) = delete; test_t(test_t&&) = delete; };
