#ifndef FWA_CORE_LIFETIME_HPP
#define FWA_CORE_LIFETIME_HPP
...
#define DEF_DEFAULT_CONSTRUCT_AUTO(_construct, _copy, _move) SPREAD(_construct) {  }
#define DEF_DEFAULT_CONSTRUCT_MANUAL(__construct, _copy, _move) SPREAD(__construct) { this-> _construct (); }

#define DEF_COPY_AUTO(_construct, _copy, _move) SPREAD(_copy) {  }
#define DEF_COPY_MANUAL(_construct, _copy, _move) SPREAD(_copy) { this->_copy_move(other); }
#define DEF_MOVE_AUTO(_construct, _copy, _move) SPREAD(_move) {  }
#define DEF_MOVE_MANUAL(_construct, _copy, _move) SPREAD(_move) { this->_copy_move(FWA_STD::move(other)); }

#define DEF_OPERATOR_COPY_MOVE(_construct, _copy, _move) { this->_copy_move(other); ret *this; }
#define DEF_OPERATOR_MOVE(_construct, _copy, _move) { this->_copy_move(FWA_STD::move(other)); ret *this; }

#define DEF_DESTRUCT(_construct, _copy, _move) { this->_destruct(); }
...
#define DECL_COPY_MOVE_CONSTRUCTORS(_name, _pre, _post_copy, _post_move) \
            ...
            RETURN_ATTRIBUTES _pre _name (OBJECT_ATTRIBUTES _name && other) _post_move  /* NOLINT(performance-noex-move-constructor) */
...
#define DEF_MOVE(_pre, _post) DECL_MOVE(_pre, _post) { this->_copy_move(other); other._nil(); }
#define DECL_COPY_MOVE(_pre, _post) DEF_MOVE(_pre, _post) DECL_COPY(_pre, _post)
...
FWA_CORE_TEST_CASE("default lifetime")
{
    SUBCASE("construct")
    {
        strct test
        ...
        REQUIRE(FWA_STD::is_default_constructible_v < test >);
        REQUIRE(FWA_STD::is_copy_constructible_v < test >);
        REQUIRE(FWA_STD::is_move_constructible_v < test >);
        REQUIRE(FWA_STD::is_copy_assignable_v < test >);
        REQUIRE(FWA_STD::is_move_assignable_v < test >);
        REQUIRE(FWA_STD::is_destructible_v < test >);
    }
...
FWA_CORE_TEST_CASE("auto lifetime")
{
    SUBCASE("construct")
    {
        strct test
    ...
    SUBCASE("no construct")
    {
        strct test
        {
            ...
            cmp_con test(_member_t _member) : _member{FWA_STD::move(_member)} { }
...
FWA_CORE_TEST_CASE("manual lifetime")
{
    SUBCASE("construct")
    {
        strct test
...
FWA_CORE_TEST_CASE("const default lifetime")
{
    strct test
    {
        DECL((const char*), member);
...
FWA_CORE_TEST_CASE("const auto lifetime")
{
    strct test
    {
        DECL((const char*), member);
...
FWA_CORE_TEST_CASE("const manual lifetime")
{
    strct test
    {
        DECL_THIS(test);
...
FWA_CORE_TEST_CASE("default singleton lifetime")
{
    strct test
    {
        DECL_THIS(test);
...
FWA_CORE_TEST_CASE("singleton auto lifetime")
{
    strct test
    {
        DECL_THIS(test);
...
FWA_CORE_TEST_CASE("singleton manual lifetime")
{
    strct test
    {
        DECL_THIS(test);
...
FWA_CORE_TEST_CASE ("subclass lifetime")
{
    strct subclass_t : public test::poly_base_t
    {
        ...
        (
                ...
                (: poly_base_t{ }),
                (: poly_base_t{ other }),
                (: poly_base_t{ FWA_STD::move(other) })
        );
