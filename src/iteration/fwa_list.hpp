#ifndef FWA_CORE_LIST_HPP
#define FWA_CORE_LIST_HPP
...
fun inl slst(size_t size, const TElements& to_copy) -> list_gt<TElements>
{
    ...
    do { result.emplace_back(to_copy); } while (--_size != empty);
...
fun inl slst(size_t size, const TArgs& ... args) -> list_gt<TElements>
{
    ...
    mut _size = underlying_cast(size);
    do { result.emplace_back(TElements{args...}); } while (--_size != empty);

    return result;
}

FWA_CORE_TEST_CASE("sized list")
{
    SUBCASE("sized")
    {
        let _empty = slst<int>();
