...
set(__env_delimiter_regex
    [[\.|-|_|/|\\|::]]
    CACHE STRING "Matches delimiters." FORCE)

set(__env_extension_regex
    [[\..*$]]
    CACHE STRING "Matches file extensions." FORCE)
...
if(CMAKE_BUILD_TYPE STREQUAL Debug)
  option(ENV_LOG_VERBOSE "Turn on verbose CMake logging." ON)
else()
  option(ENV_LOG_VERBOSE "Turn on verbose CMake ON." OFF)
endif()
...
"
    CACHE STRING "Message levels considered verbose by the environment." FORCE)
...
"
    CACHE STRING "Valid message levels for environment." FORCE)
...
set(ENV_CLANG_CL
    FALSE
    CACHE BOOL "Whether ClangCL was detected or not.")
set(ENV_MSVC
    FALSE
    CACHE BOOL "Whether MSVC was detected or not.")
set(ENV_GNU
    FALSE
    CACHE BOOL "Whether GCC was detected or not.")
set(ENV_CLANG
    FALSE
...
set(__env_cxx_byte_order
    ...
    CACHE STRING "CXX byte order." FORCE)
set(__env_c_byte_order
    "${CMAKE_C_BYTE_ORDER}"
...
set(ENV_32BIT
    FALSE
    CACHE BOOL "Whether architecture is 32 bit.")
set(ENV_64BIT
    FALSE
    CACHE BOOL "Whether architecture is 64 bit.")

set(ENV_CXX_BYTE_ORDER
    "${__env_cxx_byte_order}"
    CACHE STRING "CXX byte order." FORCE)
set(ENV_C_BYTE_ORDER
    "${__env_c_byte_order}"
    CACHE STRING "C byte order." FORCE)
...
set(ENV_CXX_BIG_ENDIAN
    FALSE
    CACHE BOOL "Whether architecture is big endian.")
set(ENV_CXX_LITTLE_ENDIAN
    FALSE
...
set(ENV_AMD64
    FALSE
    CACHE BOOL "Whether architecture is amd64.")
set(ENV_X86
    FALSE
    CACHE BOOL "Whether architecture is x86.")
set(ENV_ARM
    FALSE
    CACHE BOOL "Whether architecture is arm.")
set(ENV_ARM64
    FALSE
    CACHE BOOL "Whether architecture is arm64.")
...
set(ENV_WIN
    FALSE
    CACHE BOOL "Whether OS is Windows.")
set(ENV_LINUX
    FALSE
    CACHE BOOL "Whether OS is Linux.")
set(ENV_MACOS
    FALSE
    CACHE BOOL "Whether OS is macOS.")

set(ENV_ANDROID
    FALSE
    CACHE BOOL "Whether OS is Android.")
set(ENV_IOS
    FALSE
    CACHE BOOL "Whether OS is iOS.")
...
set(__env_binary_root
    "${CMAKE_SOURCE_DIR}/.bin"
    CACHE STRING "Root of all binary target outputs." FORCE)

set(__env_binary_prefix
    "${ENV_OS}_${ENV_ARCH}_${ENV_COMPILER}"
    CACHE STRING "\
Prefix that encodes OS, architecture and compiler information into a \
target binary output file." FORCE)
...
function(env_target_set_bin_output _target)
  ...
  if(_type STREQUAL MODULE_LIBRARY)
    env_target_set(${_target} LIBRARY_OUTPUT_DIRECTORY "${_dir}" OUTPUT_NAME
                   "${_name}")
  ...
  elseif(_type STREQUAL STATIC_LIBRARY)
    env_target_set(${_target} ARCHIVE_OUTPUT_DIRECTORY "${_dir}" OUTPUT_NAME
                   "${_name}")
  elseif(_type STREQUAL EXECUTABLE)
    env_target_set(${_target} RUNTIME_OUTPUT_DIRECTORY "${_dir}" OUTPUT_NAME
                   "${_name}")
  endif()
endfunction()
...
function(env_target_get_location _target _out)
  ...
  if(_type STREQUAL MODULE_LIBRARY)
    get_target_property(_config_dir ${_target} LIBRARY_OUTPUT_DIRECTORY)
    get_target_property(_dir ${_target}
                        LIBRARY_OUTPUT_DIRECTORY_${CMAKE_BUILD_TYPE})
    env_null_set(_dir "${_config_dir}")

  elseif(_type STREQUAL SHARED_LIBRARY)
    if(ENV_WIN)
      get_target_property(_config_dir ${_target} ARCHIVE_OUTPUT_DIRECTORY)
      get_target_property(_dir ${_target}
  ...
  elseif(_type STREQUAL EXECUTABLE)
    get_target_property(_config_dir ${_target} RUNTIME_OUTPUT_DIRECTORY)
    get_target_property(_dir ${_target}
...
if(ENV_CLANG_CL OR ENV_MSVC)
  function(env_target_conform _name)
    ...
    # clang-cl and sometimes MSVC don't enable exceptions by default unlike all
    # other compilers
...
function(env_set_cpp17 _name)
  ...
  set_target_properties(${_mod} PROPERTIES CXX_EXTENSIONS OFF C_EXTENSIONS OFF)
endfunction()
...
# TODO: clang analysis deeply copy the original target and add the deep copy as
# a dependency with the --analyze flag?
...
elseif(ENV_GNU)
  function(env_target_warn _name)
    ...
    target_compile_options(
      ${_mod}
      PRIVATE -Wall -Wextra -Wpedantic -Werror -fanalyzer
...
function(env_target_clear_warn _name)
  ...
  if(NOT _options STREQUAL _options-NOTFOUND)
    ...
    set_target_properties(${_name} PROPERTIES COMPILE_OPTIONS "${_options}")
  endif()
  ...
  if(NOT _interface_options STREQUAL _interface_options-NOTFOUND)
    ...
    set_target_properties(${_name} PROPERTIES INTERFACE_COMPILE_OPTIONS
                                              "${_interface_options}")
  endif()
endfunction()
...
if(CMAKE_BUILD_TYPE STREQUAL Release OR CMAKE_BUILD_TYPE STREQUAL MinSizeRel)
  ...
  if(ENV_CLANG_CL)
    function(env_target_optimize _name)
      ...
      target_compile_options(
        ${_mod} PRIVATE /Zi
                        # -fsanitize=address,undefined /fsanitize=address
      )
    endfunction()
  elseif(ENV_MSVC)
    function(env_target_optimize _name)
...
function(env_add_objects _name)
  ...
  env_target_definitions(${_name} PUBLIC "${UPPER_PROJECT_NAME}_OBJECT=1")
...
function(env_add_static _name)
  ...
  env_target_definitions(${_name} PUBLIC "${UPPER_PROJECT_NAME}_STATIC=1")
...
function(env_add_app _name)
  ...
  env_target_definitions(${_name} PUBLIC "${UPPER_PROJECT_NAME}_APP=1")
endfunction()
...
function(env_add_test _name)
  ...
  env_target_definitions(${_name} PUBLIC "${UPPER_PROJECT_NAME}_TEST=1")
...
function(env_add_bench _name)
  if(NOT CMAKE_BUILD_TYPE STREQUAL Debug)
    ...
    env_target_definitions(${_name} PUBLIC "${UPPER_PROJECT_NAME}_BENCH=1")
  endif()
endfunction()
...
set(__env_scaffold_default_include_dirs
    "/include"
    CACHE STRING "Default include directory for scaffolded libraries.")

set(__env_scaffold_default_src_glob
    "/src/*.cpp;/src/*.cc;/src/*.c;/source/*.cpp;/source/*.cc;/source/*.c"
    CACHE STRING "Default list of globbing patterns for scaffolded libraries.")
...
set(__env_fetch_dir
    ...
    CACHE STRING "Download directory for fetching dependencies." FORCE)

set(__env_fetch_build_dir
    "${PROJECT_BINARY_DIR}/.fetch"
    CACHE STRING "Build directory for fetched dependencies." FORCE)
...
function(env_fetch _name)
  ...
  if(PARSED_INTO)
    ...
    set(_lock_file "${__env_fetch_dir}/.process/${PARSED_INTO}/${_name}.lock")
  else()
  ...
  if(PARSED_SCAFFOLD)
    env_scaffold("${_src_dir}" NAME "${_name}" BINARY_DIR "${_bin_dir}"
                 ${PARSED_SCAFFOLD})
  endif()
endfunction()
...
function(env_project)
  ...
  if(CMAKE_BUILD_TYPE STREQUAL Debug)
    option(${UPPER_PROJECT_NAME}_COMPILER_MESSAGES
           "Turn on compiler messages for ${PROJECT_NAME}." ON)

  else()
    option(${UPPER_PROJECT_NAME}_COMPILER_MESSAGES
           "Turn on compiler messages for ${PROJECT_NAME}." OFF)

  endif()

  option(${UPPER_PROJECT_NAME}_BUILD_STATIC "Build ${PROJECT_NAME} static." ON)

  option(${UPPER_PROJECT_NAME}_BUILD_SHARED "Build ${PROJECT_NAME} shared." OFF)

  option(${UPPER_PROJECT_NAME}_BUILD_OBJECTS "Build ${PROJECT_NAME} objects."
         OFF)

  option(${UPPER_PROJECT_NAME}_BUILD_APPS "Build ${PROJECT_NAME} apps." OFF)

  option(${UPPER_PROJECT_NAME}_BUILD_BINDS "Build ${PROJECT_NAME} bindings."
         OFF)

  if(${UPPER_PROJECT_NAME}_BUILD_BINDS)
    set(${UPPER_PROJECT_NAME}_BUILD_BIND_UTILS
        ON
        CACHE BOOL "Build ${PROJECT_NAME} binding utils." FORCE)
  else()
    option(${UPPER_PROJECT_NAME}_BUILD_BIND_UTILS
           "Build ${PROJECT_NAME} binding utils." OFF)
  endif()

  option(${UPPER_PROJECT_NAME}_BUILD_TESTS "Build ${PROJECT_NAME} tests." OFF)

  option(${UPPER_PROJECT_NAME}_BUILD_BENCHMARKS
         "Build ${PROJECT_NAME} benchmarks." OFF)

  option(${UPPER_PROJECT_NAME}_BUILD_CI "Build ${PROJECT_NAME} CI." OFF)
